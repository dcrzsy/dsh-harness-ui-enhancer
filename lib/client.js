 ...
 * fills the draft via inputActions.setDraft — sending stays explicit.
 */
			...
			ctx.effect(() => {
				...
				const setupCollapse = () => {
					document.querySelectorAll(".gdEzaW_userStack .gdEzaW_bubble").forEach((bubble) => {
						...
						if (!textEl) return;
						if (textEl.classList.contains("enhc-collapsed")) return;
						...
						btn.addEventListener("click", () => {
							const collapsed = textEl.classList.toggle("enhc-collapsed");
							if (collapsed) {
								textEl.style.maxHeight = maxH + "px";
								btn.textContent = "展开";
							} else {
				...
				const mo = new MutationObserver(setupCollapse);
