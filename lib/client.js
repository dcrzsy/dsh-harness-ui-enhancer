...
function PromptLibraryButton({ inputActions }) {
	...
	const closeManage = () => { setManageOpen(false); cancelEdit(); setManageFilter(""); setConfirmId(null); setSaveErr(""); setRemovedItem(null); if (undoRef.current) { window.clearTimeout(undoRef.current); undoRef.current = null; } };
	const addItem = async () => {
