...
function PromptLibraryButton({ inputActions, session }) {
	promptToolsCss();
	...
	const insert = (text) => {
		...
		setOpen(false);
		if (el) window.setTimeout(() => el.focus(), 60);
	};
	...
	const polishDraft = async () => {
		...
		setPolishErr("");
		try {
			const resp = await fetch("/polish", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ text: draftText, requirement: "", sessionId: session?.sessionId }) });
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const attachPanelRo = () => {
					...
					if (panelEl === null || panelEl === observedPanel) return;
					ro.disconnect();
					ro.observe(panelEl);
					observedPanel = panelEl;
				};
				attachPanelRo();
				// Lazy re-attach only: the panel may appear after boot (hero page has
				// none). Cheap querySelector every second; size/class changes are
				// already covered by ResizeObserver + MutationObserver.
				const t = window.setInterval(attachPanelRo, 1e3);
...
ctx.effect(() => {
			...
			ctx.effect(() => {
				...
				// per second instead of full alignPanel+applyState every 250ms.
			...
			ctx.effect(() => {
				...
				const setupCollapse = () => {
					...
					document.querySelectorAll(".gdEzaW_userStack .gdEzaW_bubble").forEach((bubble) => {
						// Per-bubble done marker: a bubble is checked once (when it first
						// appears) unless React rebuilds it, so a long session with hundreds
						// of bubbles does not re-run getComputedStyle/scrollHeight (forced
						// reflow) on every observer tick. Already-collapsed bubbles still
						// take the cheap legacy-px migration path.
						if (bubble.dataset.enhcDone === "1") {
							const te = bubble.querySelector("div");
							if (te && te.classList.contains("enhc-collapsed") && te.style.maxHeight && te.style.maxHeight.indexOf("var(") === -1) {
								te.style.maxHeight = COLLAPSE_H;
							}
							return;
						}
						if (bubble.querySelector(".enhc-expand-btn")) { bubble.dataset.enhcDone = "1"; return; }
						...
						if (textEl.scrollHeight <= maxH + 4) {
							// Looks short — mark done so we never re-scan it (avoids an
							// endless 150/600ms re-scan loop for stable short messages).
							bubble.dataset.enhcDone = "1";
							return;
						}
