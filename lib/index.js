...
function apply(ctx) {
	...
	const patchMissingPrepareCall = () => {
		try {
			...
			for (const providerEntry of providers) {
				...
				console.warn('[harness-ui-enhancer] patched prepareCall onto adapter "' + pid + '"');
			}
		} catch (error) {
			console.warn("[harness-ui-enhancer] adapter patch error:", error instanceof Error ? error.message : error);
		}
	};
	...
	if (typeof ctx.sessionTitle?.register === "function") {
		try {
			...
			const TITLE_PROVIDER_ID = "harness-ui-enhancer/title";
			...
			const selectMessages = (messages) => {
				...
				if (picked.length === 0) throw new Error("harness-ui-enhancer: title provider requires at least one human message");
				return picked;
			};
			...
			console.warn("[harness-ui-enhancer] title provider unavailable:", error instanceof Error ? error.message : error);
		}
	}
